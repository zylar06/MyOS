...
创建 256 项 IDT 表。目前注册了 0～31 号 CPU 异常处理入口；异常发生时会输出异常编号、错误码和 EIP，然后停止内核。

### `pic.c`

初始化并重映射 8259A PIC：主 PIC 映射到 `0x20`～`0x27`，从 PIC 映射到 `0x28`～`0x2F`。在对应 IRQ 处理器完成前，所有硬件 IRQ 保持屏蔽。
