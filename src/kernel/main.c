...
extern void process_user_task(void);
...
void kernel_main(unsigned int magic, unsigned int info_address)
{
    ...
    process_create(process_user_task);

    terminal_write("Initializing timer and keyboard...\n");
    timer_init();
    keyboard_init();
    terminal_write("Timer and keyboard initialized\n");
    process_yield();
    interrupts_enable();
}
