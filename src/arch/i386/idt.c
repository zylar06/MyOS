...
static void idt_set_gate_flags(uint8_t number, uint32_t base, uint8_t flags)
{
...
{
    idt_set_gate_flags(number, base, 0x8E);
}
...
void idt_init(void)
{
    ...
    // DPL=3 so user code can enter the kernel through int 0x80.
    idt_set_gate_flags(0x80, (uint32_t)syscall_entry, 0xEE);
